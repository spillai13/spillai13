Mental Model: How I Build with AI
...
If the "How" is the engine, the "What" is the destination. Once syntax is off the table, the focus shifts entirely to product judgment.
...
The Context Cliff — where the AI forgets why a decision was made three sessions ago — is the biggest risk in long-running AI-assisted projects. An automated Librarian agent watches Git diffs and chat history to keep lat.md in sync with the actual state of the code, preserving intent, not just state.
...
The IDE is not the only input surface. A voice memo on Telegram, transcribed and analyzed against the knowledge graph, should be able to produce a prioritized ticket for the tech agent to execute in VS Code. The loop extends beyond the editor.

5. Deterministic Routing (The Orchestrator)
Not every prompt warrants a full knowledge graph lookup. A Router should classify and dispatch:
